...
pub fn randomSpawnTile(rng: std.Random, cfg: config.GameConfig) types.Tile {
    return randomSpawnTileWithWeights(rng, cfg.spawn_two_weight, cfg.spawn_four_weight);
}

pub fn randomStartTile(rng: std.Random, cfg: config.GameConfig) types.Tile {
    return randomSpawnTileWithWeights(rng, cfg.start_spawn_two_weight, cfg.start_spawn_four_weight);
}

pub fn randomSpawnTileWithWeights(rng: std.Random, two_weight: u8, four_weight: u8) types.Tile {
    const total: u16 = @as(u16, two_weight) + @as(u16, four_weight);
    const n: u16 = rng.uintLessThan(u16, total);
    if (n < two_weight) return types.Tile.number(2);
    return types.Tile.number(4);
}
